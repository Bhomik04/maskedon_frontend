...
export default function EditEventPage() {
  ...
  const loadEvent = useCallback(async () => {
    ...
    try {
      ...
      const p: Event = res.data.data.event;
      setEvent(p);
      ...
      setError(getApiErrorMessage(loadError, "Failed to load event"));
    } finally {
      setPageLoading(false);
    }
  }, [eventId, user]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(getApiErrorMessage(submitError, "Failed to update event"));
    } finally {
      setSaving(false);
    }
  }
  ...
  const canEdit = event && event.status === "upcoming" && user && event.host_id === user.id;
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center pb-28 md:pb-12">
        <div className="text-center max-w-sm mx-auto px-4">
          ...
          <p className="text-text-muted text-sm mb-6">{error || "This event can no longer be edited. Only upcoming events can be modified by their host."}</p>
          <button onClick={() => navigate(`/events/${eventId}`)} className="btn-secondary-luxe px-6 py-3 rounded-xl font-bold text-sm inline-flex items-center gap-2">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 md:py-8">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <button onClick={() => navigate(`/events/${eventId}`)} className="w-10 h-10 rounded-xl bg-surface-light flex items-center justify-center text-text hover:bg-surface transition tap-active" aria-label="Back to event">
